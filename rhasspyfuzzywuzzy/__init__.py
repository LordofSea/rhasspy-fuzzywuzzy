...
def extract_one_sqlite(query: str, examples_path: str):
    ...
    c.execute("SELECT name FROM intents ORDER BY rowid LIMIT 1 OFFSET ?", (result[2],))
    name_of_intent = c.fetchone()[0]

    conn.close()

    return (result[0][0], best_path, result[1], name_of_intent)
...
def recognize(
    ...
    best_text, best_path, best_score, name_of_intent = extract_one_sqlite(fuzz_utils.default_process(input_text), examples_path)
